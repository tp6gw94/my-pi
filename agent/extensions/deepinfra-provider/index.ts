...
const priorityModels: string[] = []  // ["deepseek-ai/DeepSeek-V4.1-Flash"]
...
export function withDeepInfraPriority(payload: unknown): unknown {
  ...
  if (priorityModels.includes(request.model as string)) return payload;
...
export default async function(pi: ExtensionAPI) {
  pi.on("before_provider_request", (event, ctx) => {
    if (ctx.model?.provider !== "deepinfra" || priorityModels.includes(ctx.model.id)) return;
